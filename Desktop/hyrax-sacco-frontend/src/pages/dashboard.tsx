...
const statBox = (label: string, value: string, color: string) => (
  <div style={{ textAlign: "center", padding: "0.75rem" }}>
    ...
    <p style={{ fontSize: "20px", fontWeight: 700, color, margin: "4px 0 0" }}>
      {value}
    </p>
  </div>
);
  ...
  return (
    <div style={{ maxWidth: "800px", margin: "0 auto" }}>
      ...
      {card(
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
            ...
            <span style={{
              background: healthColor(data.financialHealth.rating),
          ...
          <div style={{ background: "#eee", borderRadius: "4px", height: "8px", marginBottom: "0.75rem" }}>
            <div style={{
              background: healthColor(data.financialHealth.rating),
              width: `${data.financialHealth.score}%`,
              height: "100%", borderRadius: "4px",
              transition: "width 0.5s"
            }} />
          </div>
          {data.financialHealth.breakdown.map((item, i) => (
            <p key={i} style={{ fontSize: "13px", color: "#555", margin: "4px 0" }}>
              {item}
            </p>
          ))}
        </>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1rem" }}>
        ...
        {card(
          <>
            <p style={{ fontWeight: 700, color: GREEN, margin: "0 0 0.75rem", fontSize: "14px" }}>
              💰 Savings
            </p>
            <p style={{ fontSize: "22px", fontWeight: 700, color: BLACK, margin: "0 0 4px" }}>
              {fmt(data.savings.balance)}
            </p>
            <p style={{ fontSize: "12px", color: "#888", margin: "0 0 0.75rem" }}>
              Current balance
            </p>
            <div style={{
              background: data.savings.thisMonthPaid ? "#f0faf5" : "#fef2f2",
              borderRadius: "6px", padding: "8px 10px", marginBottom: "0.5rem"
            }}>
        ...
        {card(
          <>
            ...
            <p style={{ fontSize: "22px", fontWeight: 700, color: BLACK, margin: "0 0 4px" }}>
              {data.shares.totalUnits} units
            </p>
            <p style={{ fontSize: "12px", color: "#888", margin: "0 0 0.75rem" }}>
              {fmt(data.shares.totalValue)} total value
            </p>
            <div style={{
              background: data.shares.meetsMinimum ? "#f0faf5" : "#fef2f2",
              borderRadius: "6px", padding: "8px 10px", marginBottom: "0.5rem"
            }}>
              <p style={{ fontSize: "12px", color: data.shares.meetsMinimum ? GREEN : RED, margin: 0, fontWeight: 600 }}>
                {data.shares.meetsMinimum
                  ? "✅ Meets minimum"
                  : `❌ Shortfall: ${fmt(data.shares.shortfall)}`}
              </p>
            </div>
            <p style={{ fontSize: "12px", color: "#888", margin: "4px 0" }}>
          ...
          {data.loans.activeLoan ? (
            <>
              ...
              <div style={{ background: "#eee", borderRadius: "4px", height: "8px", marginBottom: "6px" }}>
                <div style={{
                  background: GREEN,
                  width: data.loans.activeLoan.percentPaid,
                  height: "100%", borderRadius: "4px"
                }} />
              </div>
              <p style={{ fontSize: "12px", color: GREEN, margin: "0 0 0.5rem" }}>
                {data.loans.activeLoan.percentPaid} repaid
              </p>
              <p style={{ fontSize: "12px", color: "#888", margin: "4px 0" }}>
                Monthly installment: {fmt(data.loans.activeLoan.monthlyInstallment)}
              </p>
              <p style={{ fontSize: "12px", color: "#888", margin: "4px 0" }}>
                Due: {new Date(data.loans.activeLoan.dueDate).toLocaleDateString("en-KE", {
            ...
            <>
              <p style={{ color: "#888", fontSize: "13px", margin: "0 0 0.75rem" }}>
                No active loan
              </p>
              <div style={{
                background: data.loans.eligibility.canBorrow ? "#f0faf5" : "#fef2f2",
              ...
              {data.loans.eligibility.canBorrow && (
                <div style={{ marginTop: "0.75rem" }}>
                  ...
                  <p style={{ fontSize: "12px", color: "#555", margin: "2px 0" }}>
                    Normal: {fmt(data.loans.eligibility.maxLoanByProduct.normal)}
                  </p>
                  <p style={{ fontSize: "12px", color: "#555", margin: "2px 0" }}>
                    Emergency: {fmt(data.loans.eligibility.maxLoanByProduct.emergency)}
                  </p>
                  <p style={{ fontSize: "12px", color: "#555", margin: "2px 0" }}>
                    School fees: {fmt(data.loans.eligibility.maxLoanByProduct.schoolFees)}
                  </p>
                </div>
              )}
            </>
        ...
        <>
          ...
          {data.upcomingObligations.map((ob, i) => (
            <div key={i} style={{
              ...
              borderBottom: i < data.upcomingObligations.length - 1 ? "1px solid #eee" : "none"
            }}>
              <div>
                <p style={{ fontSize: "13px", color: BLACK, fontWeight: 600, margin: 0 }}>
                  {ob.type}
                </p>
              ...
              <div style={{ textAlign: "right" }}>
                <p style={{ fontSize: "13px", fontWeight: 700, color: BLACK, margin: 0 }}>
                  {fmt(ob.amount)}
                </p>
                <span style={{
                  background: ob.status === "PAID" ? GREEN : ORANGE,
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
            {[
              ...
              <div key={label}>
                ...
                <p style={{ fontSize: "13px", color: BLACK, fontWeight: 600, margin: "2px 0 0" }}>
                  {value}
                </p>
              </div>
            ))}
